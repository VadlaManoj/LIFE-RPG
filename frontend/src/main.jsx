...
const request=async(path,opt={})=>{const headers={...(opt.body instanceof FormData?{}:{'Content-Type':'application/json'}),...(opt.headers||{})};const token=localStorage.getItem('liferpg_token');if(token)headers.Authorization=`Bearer ${token}`;const r=await fetch(API+path,{...opt,headers});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.detail||'Something went wrong');return d};
...
function Dashboard({dash,setPage,refresh}){const q=dash.quests.find(x=>x.status==='available'||x.status==='in_progress');const [gm,setGm]=useState(null);const [rec,setRec]=useState(null);const [deadlines,setDeadlines]=useState([]);useEffect(()=>{request('/game-master').then(setGm).catch(()=>{});request('/recommendations').then(setRec).catch(()=>{});request('/integrations/calendar/deadlines').then(setDeadlines).catch(()=>{})},[dash.user.xp,dash.user.streak]);const urgentDeadline=deadlines.find(d=>d.is_deadline||d.urgency==='urgent');return <div className="page">{urgentDeadline&&<div className="deadlineBanner"><div className="deadlineIcon"><AlertTriangle/></div><div className="grow"><div className="eyebrow" style={{color:'var(--gold)'}}>CALENDAR DEADLINE DETECTED · {urgentDeadline.provider?.toUpperCase()||'CALENDAR'}</div><h3>{urgentDeadline.summary}</h3><p>{urgentDeadline.description||'Approaching deadline detected from connected calendar. Prioritize your related quest deliverables.'}</p></div><div className="deadlineTime"><Clock size={13}/><span>{new Date(urgentDeadline.start_time).toLocaleString(undefined,{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}</span></div></div>}{rec?.top_recommendation&&<div className="recommendationHero"><div className="recIconBox"><Sparkles/></div><div className="grow"><div className="eyebrow">{rec.headline}</div><h3>{rec.top_recommendation.title}</h3><p>{rec.top_recommendation.reason} <b>{rec.top_recommendation.suggested_action}</b></p></div><button className="primary" onClick={()=>setPage('quests')}>Take Action <ArrowRight size={14}/></button></div>}<section className="commandHero"><div><div className="eyebrow">TODAY'S ADVENTURE</div><h1>Turn real life into <em>progress.</em></h1><p>{gm?.message||'Your campaign is ready. One meaningful action is enough to move the world forward.'}</p><div className="actions"><button className="primary" onClick={()=>q?setPage('quests'):setPage('goals')}><Swords/> {q?'Play next quest':'Create campaign'}</button><button className="secondary" onClick={()=>setPage('missions')}><CalendarDays/> Today</button></div></div><HeroSnapshot dash={dash}/></section><div className="statGrid"><Stat icon={Flame} label="Streak" value={`${dash.user.streak} days`} sub="Consistency chain"/><Stat icon={Swords} label="Next Quest" value={q?'Ready':'—'} sub={q?.title||'Create a goal'}/><Stat icon={Brain} label="Skills" value={dash.skills.length} sub="Growing abilities"/><Stat icon={Trophy} label="Achievements" value={`${dash.achievements.filter(a=>a.unlocked).length}/${dash.achievements.length}`} sub="Milestones unlocked"/></div><div className="sectionHead"><div><small>YOUR COMMAND CENTER</small><h2>What should I do now?</h2></div><button onClick={()=>setPage('quests')}>All quests <ArrowRight/></button></div><div className="focusGrid"><FocusCard icon={Swords} title="Main Quest" text={q?.title||'Create your first campaign'} meta={q?`${q.estimated_minutes} min · ${q.difficulty}/5 difficulty`:'Start with one goal'} onClick={()=>setPage(q?'quests':'goals')}/><FocusCard icon={Brain} title="Skill Quest" text={dash.skills.sort((a,b)=>a.progress-b.progress)[0]?.name||'Your priority skill'} meta="Strengthen your weakest skill" onClick={()=>setPage('skills')}/><FocusCard icon={Crown} title="Next Boss" text={dash.campaign?.boss_name||'Final Boss'} meta="Build toward your campaign victory" onClick={()=>setPage('boss')}/></div><div className="twoCol"><div className="panel"><PanelHead title="Life map" icon={Activity} action="Analytics" onClick={()=>setPage('analytics')}/>{dash.categories.map(c=><div className="meter" key={c.name}><span>{c.name}</span><div><i style={{width:`${c.progress}%`}}/></div><b>{c.progress}%</b></div>)}</div><div className="panel gm"><PanelHead title="AI Game Master" icon={Sparkles}/><div className="gmMessage"><div className="gmAvatar"><BotIcon/></div><div><strong>{gm?.headline||'Your next level is built one quest at a time.'}</strong><p>{gm?.next_action||'Choose one focused action today.'}</p><small>{gm?.tone||'Adventure mode'} · Recent score {gm?.recent_score||0}%</small></div></div></div></div></div>}
