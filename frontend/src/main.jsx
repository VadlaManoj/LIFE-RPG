...
function Landing({onEnter}){
 ...
 useEffect(()=>{const a=[...document.querySelectorAll('.cinematicLanding [data-reveal]')];const o=new IntersectionObserver(e=>e.forEach(x=>{if(x.isIntersecting)x.target.classList.add('is-visible')}),{threshold:.12,rootMargin:'0px 0px -60px'});a.forEach(x=>o.observe(x));return()=>o.disconnect()},[]);
 const go=id=>{document.getElementById(id)?.scrollIntoView({behavior:'smooth',block:'start'});setMenu(false)};
 return <div className="landingPage cinematicLanding">
  <div className="landingNoise"/><div className="landingProgress"/>
  <header className="landingNav">
   <button className="landingLogo" onClick={()=>window.scrollTo({top:0,behavior:'smooth'})}><Gamepad2/> LIFE<span>RPG</span></button>
   <nav className={menu?'open':''}><button onClick={()=>go('experience')}>EXPERIENCE</button><button onClick={()=>go('features')}>SYSTEM</button><button onClick={()=>go('how')}>HOW IT WORKS</button><button onClick={()=>go('pricing')}>START</button></nav>
   <button className="landingGrab" onClick={onEnter}>ENTER THE WORLD <ArrowRight/></button><button className="landingMenu" onClick={()=>setMenu(!menu)}><Menu/></button>
  </header>
  <main>
   <section className="landingHero" id="experience">
    <div className="heroCopy" data-reveal><div className="heroKicker"><Sparkles/> PERSONAL AI LIFE CAMPAIGN</div><h1>Gamify<br/><span>your life.</span></h1><p className="heroLead">Your goals become quests. Your actions become XP. Your growth becomes a world you can see.</p><div className="heroActions"><button className="landingPrimary" onClick={onEnter}>START YOUR CAMPAIGN <ArrowRight/></button><button className="landingText" onClick={()=>go('features')}>EXPLORE THE SYSTEM <ChevronRight/></button></div><div className="heroMeta"><span><i/> AI-GENERATED CAMPAIGNS</span><span><i/> REAL-WORLD PROGRESSION</span></div></div>
    <div className="heroStage" data-reveal><div className="heroOrbit orbitOne"/><div className="heroOrbit orbitTwo"/><div className="heroGlow"/><div className="heroLevel"><span>01</span> LEVEL UP YOUR LIFE</div><div className="characterArt" onClick={onEnter} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' ')onEnter()}} aria-label="Enter the world"><div className="characterScan"/><div className="characterHead">⚔</div><div className="characterBody"><span>XP</span><b>∞</b><span>QUEST</span></div><div className="characterCore"/><div className="avatarInteractHint">CLICK TO ENTER</div></div><div className="floatingCard cardOne"><span>QUEST COMPLETE</span><b>+120 XP</b><small>REAL WORLD ACTION</small></div><div className="floatingCard cardTwo"><span>SKILL UNLOCKED</span><b>Focus +1</b><small>LEVEL 07 → 08</small></div><div className="floatingCard cardThree"><span>AI GAME MASTER</span><b>Next move ready</b><small>ADAPTIVE CAMPAIGN</small></div></div>
    <div className="heroScrollHint"><span>SCROLL TO ENTER</span><i/></div>
   </section>

   <section className="manifestoSection" data-reveal><div className="manifestoIndex">01 / WHY LIFE RPG</div><div><p className="manifestoBig">Real life already has the adventure.</p><p className="manifestoBig muted">We give it a game loop.</p></div></section>

   <section className="storySection" id="features">
    <div className="storyIntro" data-reveal><div><span className="sectionEyebrow">✦ THE GAME LOOP</span><h2>Your life,<br/><em>but playable.</em></h2></div><p>LIFE RPG turns goals, habits and ambitions into an adaptive campaign with visible progress, meaningful quests and an AI Game Master that keeps your next move clear.</p></div>
    <div className="featureRail">
     <div data-reveal><FeaturePanel tag="01 · SETUP" title="Build your character" text="Tell the Game Master what matters. Your goals become the foundation of your personal campaign." visual="character"/></div>
     <div data-reveal><FeaturePanel tag="02 · IDENTITY" title="Watch yourself evolve" text="Levels, XP, skills and attributes change as you take real-world action." visual="stats"/></div>
     <div data-reveal><FeaturePanel tag="03 · QUESTS" title="Make progress playable" text="Small actions become daily missions. Complete them, submit evidence and keep your campaign moving." visual="quests"/></div>
     <div data-reveal><FeaturePanel tag="04 · BOSS BATTLE" title="Face the hard days" text="Turn procrastination, distraction and difficult milestones into encounters you can overcome." visual="boss"/></div>
    </div>
   </section>

   <section className="whatSection" id="how"><div className="sectionCenter" data-reveal><span className="sectionEyebrow">✦ WHAT IS LIFE RPG?</span><h2>Real life.<br/><em>RPG rules.</em></h2><p>One system connecting ambition, action, evidence, learning and progression.</p></div><div className="loopGrid" data-reveal><LoopStep n="01" title="Choose a goal" icon={<Target/>}/><LoopStep n="02" title="AI builds your campaign" icon={<Sparkles/>}/><LoopStep n="03" title="Complete real quests" icon={<Swords/>}/><LoopStep n="04" title="Earn XP & level up" icon={<Zap/>}/></div></section>

   <section className="dashboardReveal"><div className="dashboardIntro" data-reveal><span className="sectionEyebrow">✦ YOUR WORLD</span><h2>A command center<br/>for your <em>real life.</em></h2><p>Goals, quests, skills, bosses and progress — designed as one living campaign.</p></div><div className="mockDashboard" data-reveal><div className="mockTop"><b>☷ LIFE RPG</b><span>CAMPAIGN // ACTIVE</span><i>LVL 07</i></div><div className="mockBody"><aside><b>YOUR HERO</b><div className="mockAvatar">⚔️</div><strong>THE ADVENTURER</strong><small>2,450 XP</small><div className="mockBar"><i/></div><span>FOCUS · 72</span><span>DISCIPLINE · 64</span><span>CREATIVITY · 81</span></aside><div className="mockMain"><div className="mockHeading"><div><small>ACTIVE CAMPAIGN</small><h3>Become a stronger developer</h3></div><button onClick={onEnter}>OPEN WORLD <ArrowRight/></button></div><div className="mockCards"><MockQuest title="Ship one project" xp="+150 XP"/><MockQuest title="Practice system design" xp="+90 XP"/><MockQuest title="Read 20 pages" xp="+60 XP"/></div><div className="mockBoss"><div><small>NEXT BOSS</small><h3>PROCRASTINATION</h3></div><div className="hp"><span/><span/><span/><span/><span/></div><b>42% HP</b></div></div></div></div></section>

   <section className="pricingSection" id="pricing"><div className="sectionCenter" data-reveal><span className="sectionEyebrow">✦ START YOUR CAMPAIGN</span><h2>Stop planning.<br/><em>Start playing.</em></h2><p>Enter the world and turn one meaningful goal into your first campaign.</p></div><div className="priceGrid" data-reveal><PriceCard label="PREVIEW" title="Free" text="Start building your character and experience the core loop." features={["Character creation","Daily quests","Basic XP progression"]} onClick={onEnter}/><PriceCard label="HERO" title="LIFE RPG" text="Unlock the full campaign experience." features={["Adaptive AI campaigns","Skills & boss battles","Analytics & weekly review","Evidence-based progression"]} featured onClick={onEnter}/><PriceCard label="FUTURE" title="Guild" text="Shared progression for future community play." features={["Community features","Shared challenges","Future integrations"]} onClick={onEnter}/></div></section>
  </main>
  <footer className="landingFooter"><div><button className="landingLogo"><Gamepad2/> LIFE<span>RPG</span></button><p>Make your real life worth playing.</p></div><div><b>EXPLORE</b><button onClick={()=>go('features')}>System</button><button onClick={()=>go('how')}>How it works</button><button onClick={()=>go('pricing')}>Start</button></div><div><b>ACCOUNT</b><button onClick={onEnter}>Enter the world</button><button onClick={onEnter}>Create your hero</button></div></footer>
 </div>
}
...
function Missions({refresh}){const [rows,setRows]=useState([]),[msg,setMsg]=useState('');const load=()=>request('/missions').then(setRows);useEffect(()=>{load()},[]);const done=async id=>{const r=await request('/missions/complete',{method:'POST',body:JSON.stringify({mission_id:id})});setMsg(`+${r.xp} XP · +${r.coins} coins`);load();refresh()};return <div className="page"><PageTitle eyebrow="DAILY MISSIONS" title={<>Small actions. <em>Big momentum.</em></>} text="A focused daily board keeps your campaign moving without becoming another giant checklist."/><div className="missionBoard">{rows.map(m=><div className={`mission ${m.status==='completed'?'done':''}`} key={m.id}><div className="iconBox"><CalendarDays/></div><div className="grow"><small>{m.mission_type.toUpperCase()} · +{m.xp} XP</small><h3>{m.title}</h3><p>{m.description}</p></div><button disabled={m.status==='completed'} onClick={()=>done(m.id)}>{m.status==='completed'?<Check/>:<Play/>}</button></div>)}</div>{msg&&<div className="notice">{msg}</div>}</div>}
function Bosses(){
